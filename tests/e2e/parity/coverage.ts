...
export const COVERAGE: Record<string, Record<string, Coverage>> = {
  'merge-pdf': {
    ...
    'Preserve bookmarks & internal links': U('no test opens the output and checks outlines or link annotations'),
    'Optional blank page between documents': U('option not exercised'),
    'Password-protected PDF support': U('merging encrypted inputs is not tested'),
  ...
  'split-pdf': {
    ...
    'Text syntax: "1-5, 8, odd, even, last"': U('the odd/even/last keywords are not exercised'),
    'ZIP output for multi-file splits': C([T('split-pdf')]),
    'Range input with live page count': U('live count not asserted'),
  },
  'compress-pdf': {
  ...
  'sign-pdf': {
    ...
    'Date stamp and initials support': U('not exercised'),
  ...
  'protect-pdf': {
    ...
    'Permission controls (print, copy, edit, etc.)': U('permission flags are not asserted in the output'),
  ...
  'unlock-pdf': {
    ...
    'Shows original encryption details': U('details panel not asserted'),
  ...
  'pdf-to-image': {
    ...
    'Custom DPI up to 600': U('the 600 DPI preset is not exercised'),
  ...
  'edit-pdf': {
    ...
    'Shapes: rectangle, ellipse, line, arrow': U('shape tools not exercised'),
    'Freehand drawing with Bezier smoothing': U('not exercised'),
    'Highlight, whiteout, and stamp tools': U('not exercised'),
    'Image and emoji overlays': U('not exercised'),
    'Undo/redo (50 actions)': U('not exercised'),
  ...
  'crop-pdf': {
    ...
    'CropBox (reversible) or Flatten (permanent)': U('the two modes are not compared in output'),
  ...
  'redact-pdf': {
    ...
    'Full metadata strip option': U('metadata is not inspected after redaction'),
  ...
  'compress-image': {
    ...
    'EXIF metadata stripping': U('output EXIF is not inspected'),
  ...
  'lock-image': {
    ...
    'PBKDF2 key derivation (100,000 iterations)': U('iteration count not asserted'),
  ...
  'view-once-image': {
    ...
    'Automatically deleted after first view': U('the burn-after-read step is not exercised'),
    ...
    'Max 10MB image size': U('the size cap is not exercised'),
  },
};
